...
void ComputeObjectShader::ExecutePipeline(ID3D12Device* id3dDevice, ID3D12GraphicsCommandList* id3dGraphicsCommandList, const std::wstring& pipelineName, const std::wstring& signatureName, Camera* camera)
{
	...
	for (int i = 0; i < m_TextureInfos.size(); ++i)
		m_TextureInfos[i].UpdateUAVInfo(id3dDevice, id3dGraphicsCommandList);
...
{
	CreateDescriptorHeap(id3dDevice, id3dGraphicsCommandList, 0, 0, 1);
...
{
	m_ThreadCount = XMUINT3(CLIENT_WIDTH, CLIENT_HEIGHT, 1);

	//m_TextureInfos.emplace_back(TEXMANAGER->GetTexture(L"DEFAULT_NORM"), 0);
	//CreateSRV(id3dDevice, m_ID3DDescriptorHeap.Get(), m_DescriptorHeapSRVStart, false);
	//++m_UnorderedAccessBufferIndex;

	m_TextureInfos.emplace_back(TEXMANAGER->CreateTexture(id3dDevice, id3dGraphicsCommandList, L"TEST_UABUFFER", CLIENT_WIDTH, CLIENT_HEIGHT, DXGI_FORMAT_R8G8B8A8_UNORM, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_UNORDERED_ACCESS), 1);
...
void ComputeObjectShader::CreateUAV(ID3D12Device* id3dDevice, ID3D12DescriptorHeap* id3dDescriptorHeap, UINT offset, bool isUsedDescriptorArray)
{
	...
	for (size_t i = m_UnorderedAccessBufferIndex; i != m_TextureInfos.size(); ++i)
		TEXMANAGER->LinkedTexture(m_TextureInfos[i].Data()->Name(), &m_TextureInfos[i]);
}

void ComputeObjectShader::ReadBackUABuffers(ID3D12GraphicsCommandList* id3dGraphicsCommandList)
{
	for (int i = m_UnorderedAccessBufferIndex; i < m_TextureInfos.size(); ++i) {
		m_TextureInfos[i].Data()->ReadBackUABuffer(id3dGraphicsCommandList);
	}
